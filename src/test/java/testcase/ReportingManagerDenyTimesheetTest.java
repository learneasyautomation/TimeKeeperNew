...
public class ReportingManagerDenyTimesheetTest extends ExtentReportClass {
    ...
    @Test(enabled = true, priority = 1, groups = {"smoke", "regression"}, description = "Verify that Time sheet is declined by Reporting Manager eventhough Project manager is approved the timesheet")
    public void testReportingManagerDenyTimesheet() throws InterruptedException {
        ...
        try {
            ...
            if (driver.getTitle().contains("Time Keeper")) {
                ...
                logger.log(LogStatus.PASS, "Reporting Manager Timesheet deny is successful" + logger.addScreenCapture(screenShotPath));
            }
        } catch (Exception e) {
            logger.log(LogStatus.ERROR, e.getMessage());
            logger.log(LogStatus.ERROR, "Exception while Reporting Manager deny timesheet"+logger.addScreenCapture(screenShotPath));
        }
    }
