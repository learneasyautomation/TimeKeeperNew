...
public class EmployeeTimeSheetApprovedStatusTest extends ExtentReportClass {
    ...
    @Test(enabled = true, priority = 1, groups = {"smoke", "regression"}, description = "Verify that status of timesheet displayed as 'Approved' for an emplyee once its approved by project manager and reporting manager")
    public void testEmployeeTimesheetApprovedStatus() throws InterruptedException {
...
//                    Assert.assertEquals(status, "Approved");
                ...
                logger.log(LogStatus.PASS, "Employee timesheet Approved status is verified successfully" + logger.addScreenCapture(screenShotPath));
            }
        } catch (Exception e) {
            logger.log(LogStatus.ERROR, e.getMessage());
            logger.log(LogStatus.ERROR, "Exception while verifying the employee timesheet Approved status"+ logger.addScreenCapture(screenShotPath));
        }
    }
