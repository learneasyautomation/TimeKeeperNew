...
public class ProjectManagerApproveTimesheetTest extends ExtentReportClass{
    ...
    @Test(enabled = true,priority = 1,groups = {"smoke","regression"}, description = "Verify that project Manager approves timesheet")
    public void testProjectManagerApproveTimesheet() throws InterruptedException {
        ...
        try {
            ...
            if (driver.getTitle().contains("Time Keeper")) {
                ...
                if(objHome.getTextMyTimeMenu().equalsIgnoreCase("My Time")){
                    ...
                    objTimeSheets.approveByProjectManager();
                }
                screenShotPath = objCaptureScreenshot.captureScreenshot(screenshotName);
                logger.log(LogStatus.PASS, "Project Manager Timesheet approval is successful" + logger.addScreenCapture(screenShotPath));
            }
        } catch (Exception e) {
            logger.log(LogStatus.ERROR, e.getMessage());
            logger.log(LogStatus.ERROR, "Exception while Project Manager approves timesheet" + logger.addScreenCapture(screenShotPath));
        }
    }
